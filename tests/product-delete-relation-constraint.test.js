...
// Legacy queued deletes must now fall back to the normal archived state.
// Permanent deletion is reserved for unused products; a referenced product
// remains restorable and hidden from the sellable catalog.
...
test('legacy queued delete archives the product when hard delete is blocked by a relation constraint', async () => {
    ...
    collection(name) {
      ...
      return {
        ...
        async update(id, data) {
          updateCalls += 1
          assert.equal(id, 'p1')
          assert.deepEqual(data, { lifecycle_status: 'archived' })
          return { id: 'p1', name: 'Widget', barcode: '123', quantity: 5, price: 10, lifecycle_status: 'archived' }
        },
      }
    },
  }
  ...
  assert.equal(updateCalls, 1, 'must archive the cloud record instead of no-opping')

  const stored = await adminDb.products.get('p1')
  assert.equal(stored.lifecycleStatus, 'archived')
  assert.equal(stored.deleted, false, 'archive lifecycle status is durable -- not a fragile local-only tombstone flag')
