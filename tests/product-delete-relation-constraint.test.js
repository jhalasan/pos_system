...
// must now fall back to archiving the cloud record instead of lying about it.
...
test('deleteProduct falls back to a durable cloud archive when hard delete is blocked by a relation constraint', async () => {
  ...
  assert.equal(stored.deleted, false, 'archived is a distinct, durable state -- not the fragile local-only "deleted" tombstone')
