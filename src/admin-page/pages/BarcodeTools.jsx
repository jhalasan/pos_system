        ...
        <section className="card barcode-card">
          <div className="panel-head">
            <div>
              ...
              <span className="sub">Generate printable barcode labels without creating or changing any product.</span>
            </div>
              ...
              {generatedBarcodes.length === 0 ? (
                ...
                <div className="barcode-row" key={barcode.id}>
                  ...
                  <div className="barcode-record">
                    <strong>{barcode.title}</strong>
                    <span>{barcode.meta}</span>
                    <BarcodePreview value={barcode.value} />
                  </div>
                  <div className="barcode-actions">
                    <button className="btn btn-outline btn-sm" onClick={() => renameGeneratedBarcode(barcode)}>
                      Rename
                    </button>
                    <button
                      className="btn btn-outline btn-sm"
                      onClick={() => handlePrintBarcode(
                        barcode.id, barcode.title, barcode.value, barcode.meta,
                      )}
                      disabled={printingBarcode === barcode.id}
                    >
                      {printingBarcode === barcode.id ? 'Printing...' : 'Print'}
                    </button>
                    <button
                      className="btn btn-outline btn-sm"
                      ...
                      disabled={printingBarcode === `save-${barcode.id}`}
                    >
                      {printingBarcode === `save-${barcode.id}` ? 'Saving...' : 'Save PDF'}
                    </button>
                    <button className="btn btn-outline btn-sm" onClick={() => deleteGeneratedBarcode(barcode.id)}>
