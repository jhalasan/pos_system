...
function toProductBarcodeLabel(record) {
  return {
    id: record.id,
    title: record.title || '',
    value: record.barcode || '',
    meta: `Generated by ${record.generatedBy || 'Admin'}`,
    createdAt: record.createdAt || new Date().toISOString(),
  }
}

function fromCloudProductBarcodeLabel(record) {
  ...
  return {
    ...
    generatedById: generatedBy?.id || firstRelation(record.generated_by) || '',
...
function assertRequiredProductFields(product) {
  if (!product.name) throw new Error('Product name is required.')
  if (!product.barcode) throw new Error('Barcode is required. Generate or enter one before saving.')
  if (!product.unit) throw new Error('Base unit is required.')
  if (!(Number(product.price) > 0)) throw new Error('Product price must be greater than zero.')
}

async function queueOperation(type, productId, payload) {
  if (type === 'updateProduct') {
    // A later edit's payload is a full snapshot that already includes
    // everything an earlier not-yet-synced edit staged, so the earlier op
    // (whatever its status — pending, conflict, or failed) is superseded.
    const superseded = await adminDb.pendingOps
      .where('productId').equals(productId)
      .filter((existingOp) => existingOp.type === 'updateProduct')
      .toArray()
    for (const existingOp of superseded) await adminDb.pendingOps.delete(existingOp.id)
  }
  const op = {
    id: newId('op'),
    type,
...
export const desktopAdminApi = {
  ...
  async updateProduct(id, data) {
    ...
    const product = {
      ...
      pendingSync: true,
      updated: new Date().toISOString(),
      // The cloud "updated" value this edit was based on, so the sync engine
      // can tell a real external conflict apart from its own earlier write.
  ...
  async productBarcodeLabels() {
    await startAdminRuntime()
    if (!(await isCloudReachable())) {
      const records = await adminDb.productBarcodeLabels.filter((record) => !record.deleted).reverse().sortBy('createdAt')
  ...
  async createProductBarcodeLabel(title) {
    ...
    if (!(await isCloudReachable())) {
      ...
      await adminDb.transaction('rw', adminDb.productBarcodeLabels, adminDb.pendingOps, async () => {
        await adminDb.productBarcodeLabels.put(local)
        await queueOperation('createProductBarcodeLabel', local.id, local)
      })
      return toProductBarcodeLabel(local)
    }

    const created = await pb.collection('product_barcode_labels').create({
      code: barcode,
      title: trimmedTitle,
      generated_by: admin.id || undefined,
    }, {
      expand: 'generated_by',
      requestKey: null,
    }).catch((error) => {
      throw new Error(pocketBaseErrorMessage(error, 'Unable to generate barcode.'))
    })

    const normalized = fromCloudProductBarcodeLabel(created)
    await adminDb.productBarcodeLabels.put(normalized)
    await recordActivity('Barcode', `Generated product barcode "${trimmedTitle}" (${barcode}).`)
    return toProductBarcodeLabel(normalized)
  },
  async renameProductBarcodeLabel(id, title) {
    await startAdminRuntime()
    const trimmedTitle = String(title || '').trim()
    if (!trimmedTitle) throw new Error('Barcode label is required.')

    if (!(await isCloudReachable())) {
      const existing = await adminDb.productBarcodeLabels.get(id)
      const updated = { ...existing, title: trimmedTitle, pendingSync: true }
      await adminDb.transaction('rw', adminDb.productBarcodeLabels, adminDb.pendingOps, async () => {
        await adminDb.productBarcodeLabels.put(updated)
        await queueOperation('renameProductBarcodeLabel', id, { title: trimmedTitle })
      })
      return toProductBarcodeLabel(updated)
    }

    const updated = await pb.collection('product_barcode_labels').update(id, {
      title: trimmedTitle,
    }, {
      expand: 'generated_by',
      requestKey: null,
    }).catch((error) => {
      throw new Error(pocketBaseErrorMessage(error, 'Unable to rename barcode.'))
    })

    const normalized = fromCloudProductBarcodeLabel(updated)
    await adminDb.productBarcodeLabels.put(normalized)
    return toProductBarcodeLabel(normalized)
  },
  async deleteProductBarcodeLabel(id) {
    await startAdminRuntime()
    if (!(await isCloudReachable())) {
      const existing = await adminDb.productBarcodeLabels.get(id)
      await adminDb.transaction('rw', adminDb.productBarcodeLabels, adminDb.pendingOps, async () => {
        if (existing) await adminDb.productBarcodeLabels.put({ ...existing, deleted: true, pendingSync: true })
        await queueOperation('deleteProductBarcodeLabel', id, {})
      })
      return null
    }

    await pb.collection('product_barcode_labels').delete(id, { requestKey: null }).catch((error) => {
      throw new Error(pocketBaseErrorMessage(error, 'Unable to delete barcode.'))
    })
    await adminDb.productBarcodeLabels.delete(id)
    return null
  },
  async cashiers() {
