...
async function nextProductBarcode() {
  ...
  const labels = await pbCollection('product_barcode_labels').catch(() => null)
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const barcode = `29${String(Date.now()).slice(-10)}${Math.floor(Math.random() * 10)}`
    const existingProduct = await products.getFirstListItem(
    ...
    const existingLabel = existingProduct || !labels ? null : await labels.getFirstListItem(
      pb.filter('code = {:barcode}', { barcode }),
      { fields: 'id' },
    ).catch((error) => {
      if (error.status === 404) return null
    ...
    if (!existingProduct && !existingLabel) return barcode
  }
...
function toProductBarcodeLabel(record) {
  ...
  return {
    ...
    title: record.title,
    value: record.code,
    meta: `Generated by ${generatedBy?.name || generatedBy?.email || 'Admin'}`,
    createdAt: record.created,
  }
}

app.get('/api/barcodes/product-labels', asyncRoute(async (_req, res) => {
  const labels = await pbCollection('product_barcode_labels')
  const records = await labels.getFullList({ sort: '-created', expand: 'generated_by' })
  res.json(records.map(toProductBarcodeLabel))
}))

app.post('/api/barcodes/product-labels', asyncRoute(async (req, res) => {
  const title = String(req.body?.title || '').trim()
  if (!title) return res.status(400).json({ error: 'Barcode label is required.' })

  const barcode = await nextProductBarcode()
  const labels = await pbCollection('product_barcode_labels')
  const created = await labels.create({
    code: barcode,
    title,
    generated_by: req.adminUser?.id || undefined,
  }, { expand: 'generated_by' })
  await createLog({
    userId: req.adminUser?.id,
    action: 'Barcode',
    detail: `Generated product barcode "${title}" (${barcode})`,
  })
  res.status(201).json(toProductBarcodeLabel(created))
}))

app.patch('/api/barcodes/product-labels/:id', asyncRoute(async (req, res) => {
  const title = String(req.body?.title || '').trim()
  if (!title) return res.status(400).json({ error: 'Barcode label is required.' })

  const labels = await pbCollection('product_barcode_labels')
  const updated = await labels.update(req.params.id, { title }, { expand: 'generated_by' })
  res.json(toProductBarcodeLabel(updated))
}))

app.delete('/api/barcodes/product-labels/:id', asyncRoute(async (req, res) => {
  const labels = await pbCollection('product_barcode_labels')
  await labels.delete(req.params.id)
  res.status(204).end()
}))
