...
doc.addSection({ children: [title, subtitle, date] });
...
while (i < n) {
  ...
  if (numMatch) {
    while (i < n && lines[i].trim().match(/^\d+\.\s+/)) {
      ...
      const p = new Paragraph({ text: item, numbering: { reference: "_num", level: 0 } });
      content.push(p);
      i++;
    }
    continue;
...
// Add content section
// Flatten content into a single section
const allChildren = content;
